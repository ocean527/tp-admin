layui.define(['layer', 'laytpl', 'utils', 'lodash', 'enhanceform'], function (exports) {
  const $ = layui.jquery,
    layer = layui.layer,
    laytpl = layui.laytpl,
    _ = layui.lodash,
    utils = layui.utils,
    _body = $('body');
  const enhanceForm = layui.enhanceform;
  ...
  Sidebar.prototype.defaults = {
    ...
    forceRefresh: false  //是否强制刷新页面，比如有的页面需要重新加载下来列表等
  };
  Sidebar.prototype.render = function (options) {
    ...
    if ($(config.elem).attr("kit-sidebar-target") !== undefined) {
        return that;
    }
    const data = {
      title: config.title,
  ...
  const _private = {
    renderHTML: function (config, data) {
      ...
      laytpl(tpl).render(data, function (html) {
        ...
        if (data.pageData) {
            setTimeout(function(){
                _private.fillFormData(_sidebar, data.pageData);
            },500);
        }
        ...
        _elem.off("click").on('click', function () {
          ...
          switch (toggle) {
            ...
            case 'off':
              ...
              _shade.show();
              //重置表单
              _private.resetFormData(_sidebar, data.pageData);
              $(this).data('toggle', 'on');
              //强制刷新表单
              if (config.forceRefresh) {
                 _sidebar.find('.kit-sidebar-reload').click();
              }
              break;
          }
        });
        ...
        _sidebar.find('.kit-sidebar-reload').on('click', function () {
          ...
          if (config.dynamicRender) {
            ...
            utils.tplLoader(url, function (data) {
                ...
                if (pageData) {
                    _private.fillFormData(_sidebar, pageData);
                }
              // 关闭加载层
    ...
    fillFormData: function (_sidebar, data) {
        if (data && utils.isObject(data) && _sidebar.find("form").length === 1) {
            var enhance = new enhanceForm({
                elem: _sidebar.find("form")
            });
            enhance.filling(data);
        }
    },
    resetFormData: function(_sidebar, data) {
        if (!data && _sidebar.find("form").length === 1) {
            _sidebar.find("form")[0].reset();
        }
    }
  };
